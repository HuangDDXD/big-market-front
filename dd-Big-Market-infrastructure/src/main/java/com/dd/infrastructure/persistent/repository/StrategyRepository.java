...
import com.dd.domain.strategy.model.valobj.*;
import com.dd.domain.strategy.repository.IStrategyRepository;
import com.dd.infrastructure.persistent.dao.*;
import com.dd.infrastructure.persistent.po.*;
...
import static com.dd.types.enums.ResponseCode.UN_ASSEMBLED_STRATEGY_ARMORY;
...
public class StrategyRepository implements IStrategyRepository {
    ...
    public RuleTreeVO queryRuleTreeVOByTreeId(String treeId) {
        ...
        for (RuleTreeNodeLine ruleTreeNodeLine : ruleTreeNodeLines) {
            ...
            List<RuleTreeNodeLineVO> ruleTreeNodeLineVOLists = ruleTreeNodeLineMap.computeIfAbsent(ruleTreeNodeLine.getRuleNodeFrom(), k -> new ArrayList<>());
            ruleTreeNodeLineVOLists.add(ruleTreeNodeLineVO);
        }
    ...
    public Boolean subtractionAwardStock(String cacheKey) {
        ...
        // 1.按照cacheKey decr 后面的值，如 99，98和key组成为库存锁的key进行使用
        // 2.枷锁为了兜底，如果后续有恢复库存，手动处理等，也不会超卖，因为所有的可用库存key，都被加锁了
        String lockKey = cacheKey + Constants.UNDERLINE + surplus;
        Boolean lock = redisService.setNx(lockKey);
        if (!lock) {
            log.error("策略奖品库存加锁失败 {}", lockKey);
        }
        return lock;
    }
